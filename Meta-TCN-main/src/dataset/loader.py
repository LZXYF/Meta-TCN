...
def _get_20newsgroup_classes(args):
    ...
    #     'the science or art of political government; mideast': 0,
    #     'a branch of knowledge or study dealing with a body of facts or truths systematically arranged and showing the operation of general laws:'
    #     'the unlimited or incalculably great three-dimensional realm or expanse in which all material objects are located and all events occur.': 1,
    #     'Sale of miscellaneous items': 2,
    #     'talk politics misc': 3,
    #     'computer graphics': 4,
    #     'science  encryption  encrypt secret': 5,
    #     'computer windows x': 6,
...
def _del_by_idx(array_list, idx, axis):
    '''
        ...
        @params: idx: will be deleted list of int
        @params: axis: int 0 表示处理维度，0 即按行删
...
def load_dataset(args):
    ...
    for ld in label_dict:
        ...
        # class_names [{'label': Int, 'text': String}, {}, ...]
        class_names.append(class_name_dic)
        # class_name_words [String(label:0), String(label:1), String(label:2), ...]
    ...
    vocab = vectors
    vocab.stoi['<pad>'] = len(vocab.stoi)
    vocab.stoi['<unk>'] = len(vocab.stoi)
    vocab.vectors = torch.cat([vocab.vectors,
                               torch.Tensor([[0 for _ in range(vocab.dim)],
                                             [random.random() for _ in range(vocab.dim)]])])
