...
class SerialSampler():
    ...
    def worker(self, done_queue, sampled_classes, source_classes):
        '''
            ...
            done_queue 是个队列
            sampled_classes 之前随机采取的 5 个样本类别 list
            source_classes 没屌用，None, 还传一路啦
        '''
        ...
        while True:
            ...
            if example_prob_metrix is None:
                ...
                for y in sampled_classes:
                    if len(self.idx_list[y]) < (self.args.shot + self.args.query):
                        tmp = np.random.choice(len(self.idx_list[y]),
                                               len(self.idx_list[y]),
                                               p=example_prob_metrix[y][0], replace=False)
                    else:
